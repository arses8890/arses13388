...
class MTProtoProxyServer:
    ...
    MTProto listens on the exact Internal Port supplied by the Telegram inbound.
    External Domain/External Port are metadata for the Telegram proxy link and are
    not read from environment variables here.
    """

    def __init__(self, inbound_id: str, port: int, sni: str = "", destination: str = "", server_name: str = ""):
        self.inbound_id = inbound_id
        # The inbound Internal Port is the ONLY source of truth for the MTProto listener.
        self.port = int(port)
